...
- `VisualUpdateType` bit values (this installed `powerbi-visuals-api`): `Data = 2`, `Resize = 4`, `ViewMode = 8`, `Style = 16`, `ResizeEnd = 32`, and higher bits `FormattingSubSelectionChange = 64`, `FormatModeChange = 128`, `FilterOptionsChange = 256`, with `All = 510` (NOT the often-quoted `62` — the enum gained bits over time). **`options.type` is a bitmask that can carry composite/undocumented values** — `126` and `254` are observed in the wild ([microsoft/PowerBI-visuals-tools#422](https://github.com/microsoft/PowerBI-visuals-tools/issues/422), filed by this repo's owner). `126 = 2|4|8|16|32|64` and `254 = 2|4|8|16|32|64|128`; both contain the Data bit (`126 & 2 === 2`, `254 & 2 === 2`), so the classifier treats them as entry-affecting. The exact high bits don't matter — only the Data bit does. **Never compare `options.type` by equality; always test specific bits with `&`.** Also note: `VisualUpdateType` is a `const enum` in the `.d.ts`, which esbuild/vitest cannot inline across the module boundary — so the orchestrator's classifier uses a literal `DATA_BIT = 2` (and its tests a local value shim) rather than referencing the enum at runtime. The existing [visual.ts:144](../../src/visual.ts#L144) Data check works in production because webpack (not esbuild) inlines the const enum.
...
- **Manual UAT (Power BI Desktop):** rebuild parity; resize-no-reload in both modes; reconcile data-refresh with stable iframe URL; measure-change re-render; runtime mode switch; raw-HTML toggle; empty↔populated transitions; scroll-position survival; dev-visual/AppSource-swap caution. **Hyperlink click behaviour after a rebuild AND a reconcile** — with delegation OFF, clicking a rendered link must do nothing (no host-frame navigation); with delegation ON, it must open via the host. This path has no automated coverage (see below), so it is a required manual check.
- **Known automated-coverage gap (from execution):** the `Visual.update()` → orchestrator → step wiring has no integration test — render-path units are tested at the domain-utils/orchestrator level, and the plan deliberately deferred full-`update()` coverage to manual UAT. During execution this gap let a hyperlink-binding ordering regression (hyperlink handlers bound pre-render instead of post-render) reach review before the code-quality reviewer caught it; it was fixed by relocating `resolveHyperlinkHandling` into the post-render finalisation of each render path. A follow-up work package could add a `CustomVisualBuilder`-driven integration test (mock host) asserting post-rebuild/reconcile hyperlink handling and iframe-node retention — worthwhile given this is a certified visual, but out of WP-B scope.
