...
describe('mapCategoricalToTable', () => {
    ...
    describe('objects (conditional formatting bags)', () => {
        it('returns objects aligned to row index from the first column that carries an objects array', () => {
            const catCol = categoryCol({ sampling: true }, 'Category', 'cq', [
                'A',
                'B'
            ]);
        ...
        it('prefers the first column (categories before values) that carries an objects array', () => {
            const catColWithObjects: any = {
                source: {
                    roles: { sampling: true },
                    displayName: 'Cat',
                    queryName: 'cq'
                ...
                values: ['X', 'Y'],
                objects: [{ templates: { rowTemplate: 'CAT' } }, undefined]
            };
            const valSource = {
                roles: { content: true },
                displayName: 'HTML',
            ...
            const categorical: any = {
                categories: [catColWithObjects],
                values: [
                    {
                        source: valSource,
                        values: ['<p>1</p>', '<p>2</p>'],
                        objects: [
                            { templates: { rowTemplate: 'VAL' } },
                            undefined
                        ]
                    }
                ]
            ...
            // Category column is first in scan order (categories before values)
            expect(result.objects[0]).toEqual({
                templates: { rowTemplate: 'CAT' }
            });
        });

        it('returns an array of undefined of length rowCount when no column has an objects array', () => {
            const catCol = categoryCol({ sampling: true }, 'Category', 'cq', [
                'A',
                'B',
                'C'
            ...
            result.objects.forEach((o) => expect(o).toBeUndefined());
        });

        it('returns empty objects array when result is empty (undefined categorical)', () => {
            const result = mapCategoricalToTable(undefined, mockHost);
